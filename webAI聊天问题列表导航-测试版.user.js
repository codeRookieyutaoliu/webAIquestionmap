...
(function () {
    ...
    const config = {
        ...
        'github.com': {
            ...
            scrollContainerSelector: 'div.overflow-y-auto, div[class*="chat-container"]'
        },
        'yuanbao.tencent.com': {
            messageSelector: 'div.agent-chat__bubble__content',
            textSelector: 'div.hyc-content-text',
            userCondition: (element) => true,
            scrollContainerSelector: 'div.overflow-auto, div[class*="message-container"]'
        },
        'chat.qwenlm.ai': {
        ...
        'copilot.microsoft.com': {
            ...
            scrollContainerSelector: 'div.overflow-auto, div[class*="chat-container"]'
        },
        'chatgpt.com': {
    ...
    function renderPage(page) {
        ...
        logger.debug(`显示问题: ${start+1} 到 ${Math.min(end, questions.length)}`);
    ...
    function loadHistoryRecords() {
        ...
        async function smartLoadHistory(attempts) {
            ...
            if (currentConfig.scrollContainerSelector) {
                logger.info(`尝试使用配置的滚动容器选择器: ${currentConfig.scrollContainerSelector}`);
                try {
                    const configuredContainers = document.querySelectorAll(currentConfig.scrollContainerSelector);
                    if (configuredContainers.length > 0) {
                        ...
                        if (bestContainer) {
                            logger.success(`找到配置指定的滚动容器: ${bestContainer.tagName}.${bestContainer.className}`);
                        }
                    }
                } catch (e) {
            ...
            // 执行滚动操作
            if (bestContainer) {
                logger.success(`使用滚动容器: ${bestContainer.tagName}.${bestContainer.className}`);
